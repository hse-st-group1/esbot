...
Our main limiting factor in our performance tests was the database inside the docker container. In this case it would be more efficient to use chaches to reduce the load on the database. In addition to server requests better it is highly recommended to use more ressources on the system to ensure fast processing times when combining the backend and the database on one single machine. To validate the database performance it is recommended to test the api alone with a in-memory database.